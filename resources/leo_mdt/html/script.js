...
function MapPanel({ hosts, onMarkerClick }) {
...
function App() {
  ...
  function handleMessage(e) {
    ...
    if (d.type === 'unitUpdate') {
      const u = d.unit
      setUnits(prev => {
        const idx = prev.findIndex(x => (x.unit_id || x.unitId) === (u.unit_id || u.unitId))
    ...
    if (d.type === 'hostAlert') {
      setAlerts(prev => [d.alert, ...prev].slice(0, 50))
    }
    if (d.type === 'permissionDenied') {
      setAlerts(prev => [{ type: 'permissionDenied', info: d.info, ts: Date.now() }, ...prev])
    }
  }

  function close() { fetch('https://leo_mdt/close', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) }) }
  ...
  function assignUnit() {
    ...
    const template = { unit_type: 'patrol', pedModel: 's_m_y_cop_01', vehicleModel: 'police', spawnCoords: selectedIncident.coords || { x: 0.0, y: 0.0, z: 0.0 }, behavior: 'drive_to_scene' }
  ...
  function refreshHosts() { fetch('https://leo_mdt/requestHosts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) }) }

  function openHostDetail(h) { fetch('https://leo_mdt/requestHostDetail', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hostId: h.hostId }) }) }

  function closeDetail() { setDetail(null) }

  function setMaintenance(hostId, enable) { fetch('https://leo_mdt/setHostMaintenance', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hostId, enable }) }); setDetail(null) }

  function setThresholds(newValues) { setThreshold(prev => ({ ...prev, ...newValues })) }
    ...
    <div id="mdt">
      ...
      <main>
          ...
          <ul>
            {incidents.map((inc) => (
              <li key={inc.incident_id || inc.id} className={selectedIncident && (selectedIncident.incident_id || selectedIncident.id) === (inc.incident_id || inc.id) ? 'selected' : ''} onClick={() => selectIncident(inc)}>
                <div className="inc-row"><div className="inc-title">#{inc.incident_id || inc.id} [{inc.type}]</div><div className="inc-detail">{inc.detail || ''}</div></div>
              </li>
            ))}
          </ul>
        </section>
        ...
        <section className="right">
          <div className="hosts-header"><h2>Hosts</h2><div className="alerts">Critical hosts: {criticalCount}</div></div>

          <MapPanel hosts={hosts} onMarkerClick={openHostDetail} />

          <div className="hosts">
            {hosts.length === 0 && <div className="empty">No hosts</div>}
            <ul>{hosts.map(h => (<HostRow key={h.hostId} h={h} onOpen={openHostDetail} threshold={threshold} />))}</ul>
          </div>
          ...
          <div className="units">
            ...
            <ul>{units.map(u => (<li key={u.unit_id || u.unitId}><div><strong>Unit {u.unit_id}</strong> — {u.unit_type} — {u.status}</div><div>Host: {u.host || u.host}</div><div>Model: {u.ped_model || u.pedModel || ''} / {u.vehicle_model || u.vehicleModel || ''}</div></li>))}</ul>
          </div>

          <div className="alerts-panel">
            <h3>Alerts</h3>
            <ul>{alerts.map((a, i) => (<li key={i}>{a.type || a.alertType} {a.host ? `host:${a.host}` : ''} {a.queued ? `queued:${a.queued}` : ''} {a.age ? `age:${a.age}s` : ''}</li>))}</ul>
          </div>
        </section>
      </main>
